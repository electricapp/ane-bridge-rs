...
/* Shared result struct written by every worker thread. */
typedef struct {
    int matmuls;
    double duration;
} WorkerResult;

/* The BNNS direct-matmul API is deprecated (macOS 13+ steers callers to
 * BNNSGraph), but it is exactly the INT8 SME/AMX path this contention probe
 * wants to exercise, so the deprecation is suppressed locally. */
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
/* BNNSNDArrayDescriptor's leading field is an enum, so `= {0}` zero-init
 * trips the C++-compat int->enum check; harmless for this BNNS-only worker. */
...
static void* bnns_int8_worker(void* arg) {
    ...
    int8_t* C = (int8_t*)aligned_alloc(64, (size_t)N * N);
    for (int i = 0; i < N * N; i++) {
        A[i] = (int8_t)((i % 11) - 5);
        B[i] = (int8_t)((i % 7) - 3);
    }
    ...
    while (!atomic_load(&g_should_stop)) {
        BNNSMatMul(false, false, 1.0f, &inA, &inB, &outC, NULL, NULL);
        count++;
    }
...
__attribute__((target("+sme2"))) static void sme_outer_product_burst(void) {
    __asm__ volatile("smstart\n"      /* enter streaming + enable ZA */
                     ...
                     :
                     :
                     : "x9", "z0", "z1", "z2", "z3", "z4", "z5", "z6", "z7", "p0", "za");
}
...
static int host_has_sme(void) {
    ...
    if (sysctlbyname("hw.optional.arm.FEAT_SME", &has, &sz, NULL, 0) == 0) {
        return has;
    }
    if (sysctlbyname("hw.optional.arm.FEAT_SME2", &has, &sz, NULL, 0) == 0) {
        return has;
    }
    return 0;
}
...
static double ane_saturation_rate(AneModel* m, AneBuffer* shared_in, int n_inflight, int batches) {
...
static AneMetrics run_under_workers(AneRequest* req, AneModel* m, AneBuffer* ib, double seconds_seq,
                                    int latency_samples, int n_inflight, int saturation_batches,
...
int main(int argc, char** argv) {
    ...
    printf("Identity model:    %zu in / %zu out bytes\n", ane_model_input_nbytes(m, 0),
           ane_model_output_nbytes(m, 0));
    ...
    AneMetrics scalar = run_under_workers(req, m, ib, SECONDS_PER_PHASE, LAT_SAMPLES, N_INFLIGHT,
                                          SAT_BATCHES, scalar_worker, NUM_WORKER_THREADS);
    ...
    AneMetrics blas = run_under_workers(req, m, ib, SECONDS_PER_PHASE, LAT_SAMPLES, N_INFLIGHT,
                                        SAT_BATCHES, blas_worker, NUM_WORKER_THREADS);
    printf("%-18s rate_seq=%7.1f/s  latency=%6.1f us  rate_sat=%7.1f/s\n",
           "BLAS sgemm workers:", blas.rate_seq, blas.latency_us, blas.rate_saturated);

    AneMetrics bnns = run_under_workers(req, m, ib, SECONDS_PER_PHASE, LAT_SAMPLES, N_INFLIGHT,
                                        SAT_BATCHES, bnns_int8_worker, NUM_WORKER_THREADS);
    printf("%-18s rate_seq=%7.1f/s  latency=%6.1f us  rate_sat=%7.1f/s\n",
    ...
    int sme_ran = 0;
    if (host_has_sme()) {
        sme = run_under_workers(req, m, ib, SECONDS_PER_PHASE, LAT_SAMPLES, N_INFLIGHT, SAT_BATCHES,
