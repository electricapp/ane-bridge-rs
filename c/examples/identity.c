...
int main(int argc, char** argv) {
    ...
    if (s != ANE_OK) { fprintf(stderr, "request create failed: %s\n", ane_last_error()); return 1; }
    ...
    for (size_t i = 0; i < n_in / sizeof(float); i++) in_buf[i] = (float)i * 0.01f;

    /* Use the byte-ptr fast path. */
    s = ane_request_set_input_bytes(req, 0, in_buf, n_in);
    if (s != ANE_OK) { fprintf(stderr, "set_input failed: %s\n", ane_last_error()); return 1; }

    /* Warm up, then time 10 runs. */
    for (int i = 0; i < 3; i++) {
        s = ane_request_run(req, ANE_QOS_DEFAULT);
        if (s != ANE_OK) { fprintf(stderr, "warmup run failed: %s\n", ane_last_error()); return 1; }
    }
    uint64_t t0 = mach_absolute_time();
    int N = 10;
    for (int i = 0; i < N; i++) {
        s = ane_request_run(req, ANE_QOS_DEFAULT);
        if (s != ANE_OK) { fprintf(stderr, "run failed: %s\n", ane_last_error()); return 1; }
    }
    double total_ms = ms_since(t0, tb);

    s = ane_request_get_output_bytes(req, 0, out_buf, n_out);
    if (s != ANE_OK) { fprintf(stderr, "get_output failed: %s\n", ane_last_error()); return 1; }
