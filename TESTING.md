...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| `integration.rs` (~20)   | Schema derived from framework, error paths, byte-ptr + zero-copy round-trips, multi-IO, concurrent threads       |
| `property.rs` (5 x 32)   | Random buffer sizes, random indices, random shapes, byte-count fuzzing through the safe wrapper                  |
| `parser_fuzz.rs` (~40)   | Adversarial `modelAttributes` fuzz (proptest) — missing keys, wrong types, NSNull, overflow dims, NSDecimal, embedded-NUL names; case count tunable via `ANE_FUZZ_CASES` |
...
| `borrow_safety.rs` (~10) | UAF on drop-after-bind, mid-flight buffer access, double-free between Rust + C, on_complete drain                |
| `race.rs` (2)            | Parallel `Model::open` from a fresh process — exercises the `dispatch_once` framework init                       |
| `stress.rs` (~10)        | Rapid create/destroy, drop-during-callback, deterministic autoreleasepool-leak check via `malloc_zone_statistics`|
