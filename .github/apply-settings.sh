#!/usr/bin/env bash
# Reconcile GitHub-side settings for electricapp/ane-bridge-rs to match
...
#
# NOTE: the default branch is currently `master`. If/when the repo is
# renamed to `main`, update the branch_protection key in settings.json and
# the BRANCH var below in lockstep.

set -euo pipefail

REPO="electricapp/ane-bridge-rs"
BRANCH="master"
...
About to apply admin-scope settings to $REPO:
  - branch protection on $BRANCH (required CI checks, no force pushes, no deletions)
...
contexts=$(jq -c ".branch_protection.${BRANCH}.required_status_checks_contexts" "$SETTINGS")
strict=$(jq -r ".branch_protection.${BRANCH}.required_status_checks_strict" "$SETTINGS")
enforce=$(jq -r ".branch_protection.${BRANCH}.enforce_admins" "$SETTINGS")
linear=$(jq -r ".branch_protection.${BRANCH}.required_linear_history" "$SETTINGS")
force=$(jq -r ".branch_protection.${BRANCH}.allow_force_pushes" "$SETTINGS")
del=$(jq -r ".branch_protection.${BRANCH}.allow_deletions" "$SETTINGS")
convres=$(jq -r ".branch_protection.${BRANCH}.required_conversation_resolution" "$SETTINGS")
...
JSON
echo "  $BRANCH: branch protection applied"
...
bp=$(gh api "repos/$REPO/branches/$BRANCH/protection")
check "bp.contexts"       "$(jq -c '.required_status_checks.contexts | sort' <<<"$bp")" \
                           "$(jq -c ".branch_protection.${BRANCH}.required_status_checks_contexts | sort" "$SETTINGS")"
check "bp.strict"         "$(jq -r '.required_status_checks.strict' <<<"$bp")" \
                           "$(jq -r ".branch_protection.${BRANCH}.required_status_checks_strict" "$SETTINGS")"
check "bp.enforce_admins" "$(jq -r '.enforce_admins.enabled' <<<"$bp")" \
                           "$(jq -r ".branch_protection.${BRANCH}.enforce_admins" "$SETTINGS")"
check "bp.linear"         "$(jq -r '.required_linear_history.enabled' <<<"$bp")" \
                           "$(jq -r ".branch_protection.${BRANCH}.required_linear_history" "$SETTINGS")"
check "bp.force"          "$(jq -r '.allow_force_pushes.enabled' <<<"$bp")" \
                           "$(jq -r ".branch_protection.${BRANCH}.allow_force_pushes" "$SETTINGS")"
check "bp.del"            "$(jq -r '.allow_deletions.enabled' <<<"$bp")" \
                           "$(jq -r ".branch_protection.${BRANCH}.allow_deletions" "$SETTINGS")"
check "bp.convres"        "$(jq -r '.required_conversation_resolution.enabled' <<<"$bp")" \
                           "$(jq -r ".branch_protection.${BRANCH}.required_conversation_resolution" "$SETTINGS")"
